...
10. Όταν δύο flows πάνω στα ίδια δεδομένα καταλήγουν σε ΔΙΑΦΟΡΕΤΙΚΗ ενέργεια (π.χ. «διάλεξε μαθητή
    για να ξεκινήσεις τώρα» vs «διάλεξε μαθητή για να προσθέσεις στη σημερινή σειρά»), προτιμάται
    ΞΕΧΩΡΙΣΤΟ, μονο-σκοπικό page component για το καθένα — χτισμένο πάνω στα ίδια μικρά reusable
    κομμάτια (π.χ. SelectableStudentRow) — αντί για ένα page component με mode flag (Sprint 5:
    SelectIndividualStudent/SelectGroupStudents παρέμειναν ανέγγιχτα, νέα AddIndividualToToday/
    AddGroupToToday flows δίπλα τους).
11. Ένα «σημείο προσοχής ανά μαθητή» έχει ΜΙΑ μοναδική πηγή αλήθειας — utils/attentionSignal.js
    (Sprint 5). Κάθε μελλοντικό σημείο της εφαρμογής που θέλει να επισημάνει κάτι σε μαθητή περνάει
    από εκεί, με τη σειρά προτεραιότητας που ήδη ορίζει το αρχείο — όχι νέα τοπική λογική.
12. Η χρωματική ταυτότητα μαθητή/ομάδας (Sprint 6, utils/scheduleColor.js) είναι ΠΑΝΤΑ παραγόμενη
    από την ταυτότητα (id/σύνολο studentIds), ΠΟΤΕ αποθηκευμένη — και ΠΟΤΕ η μοναδική πηγή
    αναγνώρισης σε μια γραμμή (πάντα μαζί με ορατό κείμενο). Εμφανίζεται ως λεπτή ένδειξη (γραμμή
    στην άκρη μιας κάρτας), ΠΟΤΕ ως γέμισμα φόντου — αυτό παραμένει δεσμευμένο για σημασιολογικά
    χρώματα κατάστασης (done/skipped/attention/mood).
13. Όταν μια οθόνη «παράγει» δεδομένα αυτόματα από κάποιο πρότυπο (π.χ. utils/scheduleGeneration.js
    → db.ensureDayGenerated, Sprint 6), η παραγωγή πρέπει να είναι φυσικά idempotent (ασφαλής να
    ξανατρέξει, ΧΩΡΙΣ marker «έτρεξε ήδη») και να ζει ΕΚΤΟΣ οποιουδήποτε useLiveQuery querier —
    το Dexie απαιτεί οι querier συναρτήσεις να είναι αυστηρά read-only (πετάει `ReadOnlyError`
    αλλιώς). Η εγγραφή γίνεται σε ξεχωριστό `useEffect`, τυλιγμένη σε Dexie transaction ώστε δύο
    ταυτόχρονες κλήσεις (π.χ. React StrictMode double-effect σε dev) να μη δημιουργούν διπλές
    εγγραφές — πραγματικό bug που βρέθηκε και διορθώθηκε στο Sprint 6.
...
**Stretched-link πάνω σε `<button>` (κρίσιμο, βρέθηκε Sprint 5):** όταν το «όλη η κάρτα clickable»
στοιχείο είναι `<button>` (ανοίγει modal/action, όχι νέο route — π.χ. SessionCard, TodayQueueItem)
αντί για `<Link>`, ΠΡΕΠΕΙ η κοινή `.stretched-link` κλάση (ui/Card.css) να μηδενίζει background/
border/padding — αλλιώς ο browser αποδίδει το προεπιλεγμένο αδιαφανές γκρι φόντο του `<button>`
ΠΑΝΩ από το περιεχόμενο της κάρτας, κρύβοντάς το οπτικά παρότι υπάρχει σωστά στο DOM. Ήδη
διορθωμένο στο ui/Card.css — μην το ξαναχτίζεις τοπικά σε νέο *Card.css.
...
## MoodPicker (ui/MoodPicker.jsx)

Τέσσερα μεγάλα κουμπιά-εικονίδια (Lucide `Laugh`/`Smile`/`Frown`/`Angry`, ΟΧΙ emoji) για προαιρετική
καταγραφή διάθεσης μαθητή στο modal ολοκλήρωσης συνεδρίας (Sprint 5) — ίδιο βάρος/θέση με τα
κουμπιά διάρκειας.

Κανόνες:
- Όλες οι τιμές (value/label/icon/variant) ορίζονται ΜΟΝΟ στο `config/moodOptions.js` — καμία
  hardcoded λίστα σε άλλο component.
- Χωρίς ορατή λεζάντα κάτω από κάθε κουμπί (μόνο `aria-label`) — σκόπιμη επιλογή προϊόντος.
- Προαιρετικό πάντα — ποτέ δεν μπλοκάρει το «Αποθήκευση». Ξανά-tap στην ήδη επιλεγμένη τιμή
  αποεπιλέγει.
- Σε ομαδική συνεδρία: ένα MoodPicker ανά (μη απόντα) μαθητή, με ετικέτα τον κωδικό του.
...
Μία γραμμή στην ενότητα «Η μέρα μου» (Sprint 5) — πρέπει, με μία ματιά, να απαντά σε τρία
ερωτήματα: ποιον βλέπω (κωδικοί + ατομικό/ομαδικό εικονίδιο) · τι πρέπει να κάνω μαζί του (το
ΠΟΛΥ ένα σήμα προσοχής, πάντα από το utils/attentionSignal.js) · τι έγινε ήδη σήμερα (ολοκληρώθηκε
+ διάθεση, αν καταγράφηκε).

Κανόνες:
- Attention signal και mood icon εμφανίζονται ΜΟΝΟ σε ατομικές γραμμές — μια ομαδική γραμμή ήδη
  δείχνει πολλούς κωδικούς μαζί, ένα signal/εικονίδιο εκεί θα ήταν ασαφές σε ποιον αναφέρεται.
- ΠΟΤΕ περισσότερα από ένα attention signals ταυτόχρονα σε μία γραμμή (βλ. § attentionSignal στο
  Γενικό κανόνα προϊόντος).
- Secondary actions (Επεξεργασία/Παράλειψη/Μετακίνηση) πάντα σε overflow menu, ποτέ inline.
- Όλη η κάρτα ανοίγει κατευθείαν Teaching Mode — stretched pattern πάνω σε `<button>` (βλ. §
  Stretched-link παρακάτω για κρίσιμο σημείο σχετικά με button vs Link).

**Sprint 6 προσθήκες** (backward-compatible — η Αρχική συνεχίζει να δουλεύει αμετάβλητη όταν αυτά
τα props απουσιάζουν): `plannedTime`/`timeState`/`timeLabel` (η ΠΡΟΓΡΑΜΜΑΤΙΣΜΕΝΗ ώρα, πάντα αμετάβλητη
ό,τι κι αν συμβεί αργότερα — utils/timeAwareness.js δίνει «Τώρα»/«Σε Χ λεπτά»/«Πέρασε» μόνο για
άλυτες γραμμές του σήμερα, καμία ένδειξη για μακρινές ώρες ώστε να μη γίνεται θόρυβος), `color`
(utils/scheduleColor.js, βλ. Γενικοί κανόνες §12), `notHeld` (διακριτή, ήπια μεταχείριση — ΟΧΙ ίδια
με το πράσινο ✓ του «ολοκληρώθηκε»), `onMarkNotHeld`/`onRestore` (η ενιαία «Επαναφορά στη σειρά»
καλύπτει ΚΑΙ το skip του Sprint 5 ΚΑΙ το notHeld του Sprint 6 με το ίδιο κουμπί/θέση στο μενού).

## SchedulePage / ScheduleSlotForm / ScheduleSlotRow (Sprint 6)

Η ενότητα «Πρόγραμμα» ανοίγει κατευθείαν στο εβδομαδιαίο πρότυπο, ΟΧΙ στο μηνιαίο ημερολόγιο (Product
Design: το πρότυπο είναι ο πραγματικός πυρήνας/σπάνια επίσκεψη, το ημερολόγιο δευτερεύον, μία σαφής
ενέργεια μακριά — βλ. DESIGN_SYSTEM.md §18β). Desktop: όλες οι μέρες-στήλες δίπλα-δίπλα (minmax(200px,
1fr) ανά στήλη — ΠΟΤΕ `1fr` σκέτο, βρέθηκε πραγματικό bug όπου το περιεχόμενο μιας γραμμής
συμπιεζόταν σε αφανές μηδενικό πλάτος χωρίς αυτό). Tablet/mobile: μία μέρα τη φορά με Tabs
επιλογέα ημέρας (διαφορετική χρήση του Tabs από τα Εβδομαδιαίο/Μηνιαίο — αυτό εναλλάσσει ΗΜΕΡΕΣ
μέσα στην ίδια οθόνη, όχι δύο διαφορετικές λειτουργίες).

Το ScheduleSlotForm ενσωματώνει τη «λειτουργία προσθήκης» (stay-in-flow: μετά την αποθήκευση δεν
κλείνει, καθαρίζει και προτείνει την επόμενη ώρα — μόνο σε δημιουργία, όχι σε επεξεργασία), την
πολλαπλή επιλογή ημερών σε ένα βήμα, και την ερώτηση «Από πότε ισχύει;» (μόνο σε επεξεργασία —
δύο επιλογές, καμία έννοια «προγραμματισμένης αλλαγής»/λίστα εκκρεμών στο UI).

## MonthlyCalendarPage / DayDetailPage / CalendarEventForm (Sprint 6)

Η λεπτομέρεια ημέρας (DayDetailPage) ΔΕΝ είναι νέο component για την ουρά της ημέρας — είναι το ΙΔΙΟ
TodayQueue, γενικευμένο με προαιρετικό `date` prop (βλ. Γενικοί κανόνες §10 πνεύμα: επαναχρησιμοποίηση
αντί για παράλληλο component). Το μηνιαίο ημερολόγιο μένει πραγματικό grid σε ΟΛΑ τα breakpoints
(όχι agenda-list σε mobile) — κάθε κελί δείχνει μόνο αριθμό + το πολύ λίγες μικρές κουκκίδες, καμία
λίστα ονομάτων· όλη η αλληλεπίδραση μετατίθεται στο DayDetailPage.
...
- «Η μέρα μου» (TodayQueue/TodayQueueItem, Sprint 5) — πρωταγωνιστής της Αρχικής, πάνω από το
  fold· η συνολική εικόνα (stats/quick actions/δραστηριότητα) παραμένει κάτω, δεύτερο επίπεδο
- MoodPicker (βλ. § MoodPicker) — Lucide εικονίδια διάθεσης, κεντρικό config, όχι hardcoded
- Ξεχωριστά μονο-σκοπικά flows αντί για mode flag σε υπάρχον component (βλ. Γενικοί κανόνες §10)
- utils/attentionSignal.js ως μοναδική πηγή αλήθειας για «ένα σημείο προσοχής ανά μαθητή» (βλ.
  Γενικοί κανόνες §11)
- Stretched-link πάνω σε `<button>` πρέπει να μηδενίζει background/border (βλ. § Card)
- «Πρόγραμμα» (SchedulePage/ScheduleSlotForm/ScheduleSlotRow, Sprint 6) — ανοίγει στο εβδομαδιαίο
  πρότυπο (όχι στο ημερολόγιο), effective-dated αλλαγές χωρίς UI για «προγραμματισμένες αλλαγές»
  (βλ. § SchedulePage)
- DayDetailPage = γενικευμένο TodayQueue με `date` prop, ΟΧΙ νέο παράλληλο component (βλ. §
  MonthlyCalendarPage/DayDetailPage)
- utils/scheduleColor.js ως μοναδική πηγή αλήθειας για χρωματική ταυτότητα μαθητή/ομάδας — παραγόμενη,
  ποτέ αποθηκευμένη, ποτέ μοναδική πηγή αναγνώρισης (βλ. Γενικοί κανόνες §12)
- Αυτόματη παραγωγή δεδομένων (π.χ. ensureDayGenerated) ΠΑΝΤΑ εκτός useLiveQuery querier, σε
  useEffect τυλιγμένη σε Dexie transaction (βλ. Γενικοί κανόνες §13)
